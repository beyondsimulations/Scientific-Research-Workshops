...
Die Tourenplanung macht einen erheblichen Anteil der Distributionskosten im Straßengüterverkehr aus. Seit das Tourenplanungsproblem (Vehicle Routing Problem, VRP) erstmals von @dantzig1959truck formalisiert wurde, haben es Fortschritte bei den Lösungsverfahren ermöglicht, immer umfangreichere Probleminstanzen zu lösen [@desaulniers2005column]. Die vorliegende Arbeit befasst sich mit dem Problem ... .
...
Die Arbeit ist wie folgt gegliedert. @sec-literatur gibt einen Überblick über die relevante Literatur. @sec-modell stellt das mathematische Modell vor, wobei @sec-annahmen die zugrundeliegenden Annahmen und @sec-formulierung die formale Formulierung enthält. @sec-rechenstudie präsentiert die Rechenstudie, @sec-diskussion diskutiert die Befunde, und @sec-zusammenfassung fasst die Ergebnisse zusammen und gibt einen Ausblick auf zukünftige Forschung.



# Literaturüberblick {#sec-literatur}
...
Spaltengenerierung hat sich als besonders effektive Technik zur Lösung umfangreicher Touren- und Ablaufplanungsprobleme erwiesen [@desaulniers2005column]. Zudem werden seit einiger Zeit Methoden des maschinellen Lernens eingesetzt, um die Leistung von Heuristiken zu verbessern [@bengio2021machine].
...
Wir definieren die folgende Notation. Mengen werden kalligraphisch gesetzt ($\mathcal{J}$), Variablen als Großbuchstaben ($X_{ij}$) und Parameter sowie Indizes als Kleinbuchstaben ($c_{ij}$, $i$, $j$).
...
<!-- HINWEIS: Eine vollständige VRP-Formulierung würde zusätzlich
     Kapazitäts- und Subtour-Eliminationsbedingungen enthalten. Sie werden
     hier weggelassen, um das Beispielmodell kurz zu halten. -->
...
Die Zielfunktion (@eq-zielfunktion) minimiert die gesamten Transportkosten. @eq-zuweisung stellt sicher, dass genau ein Fahrzeug von jedem Kunden abfährt, während @eq-fluss sicherstellt, dass genau ein Fahrzeug bei jedem Kunden ankommt. @eq-binaer definiert die binären Entscheidungsvariablen. Die Notation ist in @tbl-notation zusammengefasst.
...
Die Ergebnisse deuten darauf hin, dass die vorgeschlagene Heuristik für kleine und mittlere Instanzen gut funktioniert, jedoch bei größeren Problemgrößen zunehmende Optimalitätslücken aufweist. Dies ist konsistent mit den Beobachtungen von @toth2014vehicle, die ein ähnliches Skalierungsverhalten für konstruktive Heuristiken bei kapazitätsbeschränkten Tourenplanungsproblemen feststellten.
