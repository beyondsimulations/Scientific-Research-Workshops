---
bibliography: references.bib
bibliographystyle: apa
lang: en
format:
  typst:
    template-partials:
      - typst-show.typ
    toc: false
    toc-depth: 3
    number-sections: true
    margin:
      top: 30mm
      bottom: 20mm
      left: 40mm
      right: 25mm
    font-size: 11pt
    include-before-body:
      text: |
        // ==== TITLE PAGE — edit the fields marked with ← ====
        #page(numbering: none)[
          #v(2cm)
          #align(center)[
            #text(size: 20pt, smallcaps(strong[University of Hamburg]))
            #v(5mm)
            #text(size: 11pt)[
              Faculty of Business Administration \       // ← your faculty
              Institute of Logistics, Transport and Production // ← your institute
            ]
            #v(3cm)
            #text(size: 16pt, smallcaps(strong[Seminar Paper])) // ← Bachelor/Master/Seminar Paper
            #v(12mm)
            #text(size: 16pt, weight: "bold")[Title of the Thesis] // ← your title
            #v(12mm)
            #text(size: 11pt)[
              Independent academic work \
              for the degree of "Bachelor/Master of Science" \ // ← your degree
              in the program ...                               // ← your program
            ]
            #v(12mm)
            #text(size: 11pt)[Hamburg, #datetime.today().display()]
          ]
          #v(1fr)
          #grid(
            columns: (1fr, 1fr),
            column-gutter: 2em,
            [
              _submitted by:_ \
              #v(2mm)
              *Last Name, First Name* \    // ← your name
              #v(2mm)
              Matriculation No.: XXXXXXX \ // ← your matriculation number
              #v(2mm)
              Semester: X \                // ← your semester
              #v(2mm)
              E-Mail: first.last\@studium.uni-hamburg.de // ← your email
            ],
            [
              _supervised by:_ \
              #v(2mm)
              Prof. Dr. Knut Haase // ← your supervisor
            ],
          )
          #v(1cm)
        ]
        // ==== END TITLE PAGE ====
        #outline(depth: 3, indent: 1em)
        #pagebreak()
        #outline(title: "List of Figures", target: figure.where(kind: "quarto-float-fig"))
        #pagebreak()
        #outline(title: "List of Tables", target: figure.where(kind: "quarto-float-tbl"))
        #pagebreak()
    include-in-header:
      text: |
        #set text(lang: "en")
        #set par(leading: 1em, justify: true)
        #set math.equation(numbering: "(1)", number-align: end + horizon)
        #show heading: it => { v(0.5em) + it + v(0.2em) }
        #show figure: it => { v(0.4em) + it + v(0.4em) }
        #set table(stroke: (_, y) => (
          top: if y == 0 { 0.08em } else if y == 1 { 0.05em },
        ))
        #show table: it => block(width: auto, stroke: (bottom: 0.08em), it)
        #set bibliography(title: "References")
        #show bibliography: it => {
          pagebreak()
          it
        }
---

# List of Abbreviations {.unnumbered}

<!-- Add or remove entries. Keep alphabetical order. Define each abbreviation on first use in text. -->

LLM : Large Language Model

OR : Operations Research

VRP : Vehicle Routing Problem

{{< pagebreak >}}

# List of Symbols {.unnumbered}

<!-- Add or remove entries. Keep alphabetical order. -->

$\mathcal{I}$ : Set of customers

$\mathcal{J}$ : Set of all locations (customers and depot)

$c_{ij}$ : Travel cost from location $i$ to location $j$

$d_i$ : Demand of customer $i \in \mathcal{I}$

$x_{ij}$ : Binary variable, 1 if a vehicle travels from $i$ to $j$

{{< pagebreak >}}

<!-- ============================================================
     HOW TO USE THIS TEMPLATE

     1. Edit the title page in the YAML block above (look for ← markers)
     2. Add your references to references.bib (export from Zotero as BibTeX)
     3. Write your content below in Markdown
     4. Render: quarto render seminar-paper-en.qmd
     5. Live preview: quarto preview seminar-paper-en.qmd

     QUICK REFERENCE
     Citations:     [@key] → (Author, Year)    @key → Author (Year)
                    [@a; @b] → multiple        [@key, p. 42] → with page
     Cross-refs:    @fig-label  @tbl-label  @eq-label  @sec-label
     Figures:       ![Caption](path.png){#fig-label width="80%"}
     Tables:        pipe table + ": Caption {#tbl-label}" below
     Equations:     $$ ... $$ {#eq-label}
     Sections:      # Heading {#sec-label}

     FORMATTING BASICS
     Bold:          **bold text**
     Italic:        *italic text*
     Bold+italic:   ***bold and italic***
     Unordered list:
                    - Item 1
                    - Item 2
                      - Nested item (indent 2 spaces)
     Ordered list:
                    1. First
                    2. Second
                       a. Sub-item (indent 3 spaces)
     Footnotes:     Some text^[This is an inline footnote].
                    Or use a reference[^1] and define it later:
                    [^1]: Footnote text here.
     Links:         [link text](https://example.com)
     Inline code:   `variable_name`
     En-dash:       -- (for ranges, e.g. pages 5--10)

     TYPST BACKEND
     This template uses Typst (not LaTeX) to produce the PDF.
     For advanced formatting, you can embed raw Typst code:

         ```{=typst}
         #v(1cm)                          // vertical space
         #text(fill: red)[important]      // colored text
         #align(center)[centered text]    // alignment
         #line(length: 100%)              // horizontal line
         #set text(size: 9pt)             // change font size locally
         ```

     See https://typst.app/docs for the full Typst reference.

     COMMON MISTAKES TO AVOID
     - Every figure and table must be referenced in the text *before* it appears.
     - Figures have captions below; tables have captions above.
     - Don't start a sentence with a math symbol ("$X$ is..." → "The variable $X$ is...").
     - Be consistent with notation — don't switch between $X_{ij}$ and $X(i,j)$.
     - Avoid orphan sections (a single subsection under a section).
     - Each section or subsection should be at least one page long.
       If not, merge it with another section.
     - Define acronyms on first use, then use the abbreviation consistently.
     - Don't cite Wikipedia or lecture slides — find the original source.
     - Use en-dashes (--) for ranges (pages 5--10), not hyphens (-).
     - Spell out numbers one through twelve in running text; use digits for 13+.
     - Use a thin space before units: 16 GB, 245 s (Typst handles this automatically
       in math mode, but watch for it in running text).
     ============================================================ -->

# Introduction {#sec-introduction}

<!-- WRITING GUIDE: Introduction
     Structure your introduction as a funnel:
       1. Broad context — why does this field matter?
       2. Specific problem — what is the concrete challenge?
       3. Research gap — what is missing in the literature?
       4. Your contribution — what does this paper do?
       5. Paper outline — briefly describe the structure of the paper.
     Tip: Write this section last, once you know your actual contribution.
     It should answer: Why should the reader care? What is new here? -->

Route planning accounts for a substantial share of distribution costs in road freight transport. Since the vehicle routing problem (VRP) was first formalized by @dantzig1959truck, advances in solution methods have made it possible to solve increasingly large problem instances [@desaulniers2005column]. This paper addresses the problem of ... .

@fig-example shows a schematic overview of the problem structure.

<!-- Figure syntax: ![Caption](images/file.png){#fig-label width="80%"} -->
![Replace this with your own figure](images/Image.pdf){#fig-example width="80%"}

The remainder of this paper is structured as follows. @sec-literature reviews the relevant literature. @sec-model introduces the mathematical model, including the underlying assumptions in @sec-assumptions and the formal formulation in @sec-formulation. @sec-computational presents the computational study, @sec-discussion discusses the findings, and @sec-conclusion concludes with a summary and outlook for future research.



# Literature Review {#sec-literature}

<!-- WRITING GUIDE: Literature Review
     - Group sources by theme or method, not chronologically.
     - Don't just summarize — compare approaches, identify strengths and
       weaknesses, and show the gap your work fills.
     - Every cited work should connect to your research question.
     - End with a transition to your own contribution: what remains unsolved?
     - Use [@key] for parenthetical and @key for narrative citations. -->

The vehicle routing problem has been studied extensively since the seminal work of @dantzig1959truck. Several exact and heuristic approaches have been proposed over the decades [@toth2014vehicle].


Column generation has proven to be a particularly effective technique for solving large-scale routing and scheduling problems [@desaulniers2005column]. Recent advances in machine learning have also been applied to improve heuristic performance [@bengio2021machine].

A comprehensive overview of solution methods is beyond the scope of this paper. For a detailed survey, the reader is referred to @toth2014vehicle.


# Model {#sec-model}

<!-- WRITING GUIDE: Model
     Give a brief overview of the modeling approach before diving into details.
     Split into assumptions (what you simplify) and formulation (the math). -->

This section introduces the mathematical model for the problem described above. We first state the key assumptions, then present the formal formulation.

## Assumptions {#sec-assumptions}

<!-- WRITING GUIDE: Assumptions
     - Every assumption must be justified: why is it reasonable?
     - Acknowledge limitations: what would change if an assumption is relaxed?
     - Order from most fundamental to most specific. -->

The modeling of the problem requires several simplifying assumptions:

- A single depot serves as the starting and ending point for all vehicles.
- Each customer $i \in \mathcal{I}$ has a known demand $d_i$ that must be fully satisfied.
- The fleet consists of identical vehicles with a common capacity.
- Travel costs $c_{ij}$ between locations $i$ and $j$ are symmetric and satisfy the triangle inequality.

## Formulation {#sec-formulation}

<!-- WRITING GUIDE: Formulation
     - Define all notation before using it (see the notation table below).
     - Use consistent conventions: sets = calligraphic ($\mathcal{J}$),
       variables = uppercase ($X_j$), parameters = lowercase ($b_i$).
     - Every constraint must be explained in prose — don't just list equations.
     - Reference constraints by label: "Constraint (@eq-assignment) ensures..."
     - Table caption: ": Caption {#tbl-label}" below the table.
     - Equation label: {#eq-label} right after closing $$.
     - For constraints with a quantifier, use \qquad (2 em) before \forall
       to keep a visible gap between the expression and the index domain,
       e.g. "= 1 \qquad \forall i \in \mathcal{I}". Equation numbers are
       automatically right-aligned at the page margin.

     TABLE DESIGN TIPS
     - Use pipe tables with booktabs-style formatting (Quarto handles this).
     - Right-align numeric columns, left-align text columns.
     - Put units in column headers, not in every cell.
     - Keep tables compact — avoid unnecessary columns. -->

We define the following notation. Sets use calligraphic symbols such as $\mathcal{J}$. Variables use uppercase letters ($X_j$, $j \in \mathcal{J}$), while parameters and indices use lowercase letters ($b_i$).

| Symbol | Description |
|:------:|:------------|
| $\mathcal{I}$ | Set of customers |
| $\mathcal{J}$ | Set of all locations (customers and depot) |
| $c_{ij}$ | Travel cost from location $i$ to location $j$ |
| $d_i$ | Demand of customer $i \in \mathcal{I}$ |
| $x_{ij}$ | Binary variable, 1 if a vehicle travels from $i$ to $j$ |

: Notation overview {#tbl-notation}


<!-- NOTE: A full VRP formulation would also include vehicle capacity and
     subtour-elimination constraints. They are omitted here to keep this
     example model short. -->

The model is defined as follows:

$$
\min \sum_{i \in \mathcal{J}} \sum_{j \in \mathcal{J}} c_{ij} \cdot x_{ij}
$$ {#eq-objective}

subject to:

$$
\sum_{j \in \mathcal{J}} x_{ij} = 1 \qquad \forall i \in \mathcal{I}
$$ {#eq-assignment}

$$
\sum_{i \in \mathcal{J}} x_{ij} = 1 \qquad \forall j \in \mathcal{I}
$$ {#eq-flow}

$$
x_{ij} \in \{0, 1\} \qquad \forall i, j \in \mathcal{J}
$$ {#eq-binary}


The objective function (@eq-objective) minimizes total travel cost. @eq-assignment ensures that exactly one vehicle departs from each customer, while @eq-flow ensures that exactly one vehicle arrives at each customer. @eq-binary defines the binary decision variables. The notation is summarized in @tbl-notation.


# Computational Study {#sec-computational}

<!-- WRITING GUIDE: Computational Study
     - Report hardware (CPU, RAM), software (solver, version), and language.
     - Describe instance sources or how you generated test data.
     - Always compare against a baseline, benchmark, or exact solution.
     - Report both solution quality (gap, objective value) and computation time.
     - Discuss trends across instance sizes, don't just list individual numbers.
     - Mention time limits, number of runs, or other experimental parameters. -->

To evaluate the performance of the proposed approach, we conduct a series of computational experiments. All experiments were performed on a machine with an Intel Core i7 processor and 16 GB of RAM. The model was implemented in Python using Gurobi 11.0 as the solver.

| Instance | $|\mathcal{I}|$ | Optimal | Heuristic | Gap (%) | Time (s) |
|----------|:---------------:|--------:|----------:|--------:|---------:|
| Small-1  |       25        |   245.3 |     248.1 |    1.14 |     0.8  |
| Small-2  |       25        |   312.7 |     318.4 |    1.82 |     1.1  |
| Medium-1 |       50        | 1,340.5 |   1,382.9 |    3.16 |    12.4  |
| Medium-2 |       50        | 1,128.3 |   1,167.2 |    3.45 |    15.7  |
| Large-1  |      100        | 3,450.1 |   3,621.3 |    4.96 |   238.7  |
| Large-2  |      100        | 4,217.8 |   4,485.6 |    6.35 |   310.2  |

: Computational results comparing exact and heuristic solutions {#tbl-results}

The results in @tbl-results show that the heuristic approach finds near-optimal solutions within reasonable computation times. For small instances, the optimality gap remains below 2%, while for larger instances the gap increases to approximately 5--6%.


# Discussion {#sec-discussion}

<!-- WRITING GUIDE: Discussion
     - Interpret your results in the context of the research question.
     - Compare findings with existing literature: are they consistent?
       Where do they differ, and why?
     - Discuss limitations honestly — every study has them.
     - Distinguish observation (the data shows X) from interpretation
       (this suggests Y).
     - Address unexpected results rather than ignoring them.
     - Do not introduce entirely new topics here. New directions
       belong in the conclusion's outlook section. -->

The results indicate that the proposed heuristic performs well for small and medium instances but shows increasing gaps for larger problem sizes. This is consistent with findings by @toth2014vehicle, who observed similar scaling behavior for constructive heuristics applied to capacitated routing problems.

A key limitation of this study is the use of symmetric cost matrices, which may not hold in real-world logistics networks with one-way streets or time-dependent travel times. Furthermore, the test instances were generated randomly rather than derived from real-world data, which limits the generalizability of the results.


# Conclusion and Outlook {#sec-conclusion}

<!-- WRITING GUIDE: Conclusion
     - Distinguish summary (what you did and found) from outlook (future work).
     - Do not introduce new results or analysis here.
     - Future research directions should be concrete and actionable,
       not vague ("further research is needed").
     - Don't overclaim: use "suggests" rather than "proves",
       "indicates" rather than "demonstrates" — unless you have formal proof. -->

This paper presented a mathematical model and heuristic solution approach for the vehicle routing problem. The computational study indicates that the proposed method finds near-optimal solutions in a fraction of the computation time of the exact method.

Two directions for future research emerge from this work. The model could be extended to incorporate time windows and heterogeneous fleets. In addition, the heuristic could be enhanced with machine learning techniques to improve initial solution quality.


<!-- Bibliography is auto-generated from references.bib -->

{{< pagebreak >}}

# Declaration of Authorship {.unnumbered}

I hereby declare that I have written this work independently and without outside help, and that I have not used any sources or aids other than those indicated in the attached bibliography. All passages taken verbatim or in substance from published or unpublished sources are identified as such. All internet sources are attached to the work. Furthermore, I declare that I have not previously submitted this work in any other examination procedure and that the submitted written version corresponds to the version on the electronic storage medium.

```{=typst}
#v(2cm)
```

Hamburg, the \_\_\_\_\_\_\_\_\_\_\_\_

```{=typst}
#v(0.5cm)
```

\_\_\_\_\_\_\_\_\_\_\_\_

First Name Last Name

{{< pagebreak >}}

# Use of AI Tools {.unnumbered}

<!-- List the AI tools you used and describe how you used them.
     Be specific: which tool, for which tasks, and how you verified the output. -->

<!-- List the AI tools you used below. Add or remove entries as needed.
     Be specific about what each tool was used for.
     Examples:
     - "ChatGPT: Used for brainstorming the structure of the literature review"
     - "Claude: Used for debugging Python code in the computational study"
     - "GitHub Copilot: Used for code completion in the implementation"
     - "Grammarly: Used for proofreading and improving clarity of Section 3"
     All AI-generated content must be reviewed, verified, and revised by you. -->

The following AI tools were used during the preparation of this work:

- **Tool 1**: Used for ... .
- **Tool 2**: Used for ... .
- **Tool 3**: Used for ... .

```{=typst}
#v(2cm)
```

Hamburg, the \_\_\_\_\_\_\_\_\_\_\_\_

```{=typst}
#v(0.5cm)
```

\_\_\_\_\_\_\_\_\_\_\_\_

First Name Last Name
